...
public class Solution {
    ...
    public static void main(String[] args) throws IOException {
        Path passwords = downloadFile("https://javarush.ru/testdata/secretPasswords.txt", Paths.get("D:/testFiles"));
    ...
    public static Path downloadFile(String urlString, Path downloadDirectory) throws IOException {
        String name = urlString.replaceAll(".+/", ""); //вычисляет название файла
        String fullName = downloadDirectory.toString() + "/" + name; // вычисляет полный путь
//        System.out.println(fullName); //проверка полного имени пути

        URL url = new URL(urlString); //создаёт объект URL

        InputStream inputStream = url.openStream(); // открывает поток

        Path tempFile = Files.createTempFile("temp", ".tmp"); // создаёт временный файл

        Files.copy(inputStream, tempFile, StandardCopyOption.REPLACE_EXISTING); // копирует данный из интернета

        inputStream.close(); //закрывает поток

        Path path = Files.move(tempFile, Paths.get(fullName)); //перемещает и переименовываем файл
